...
OUTPUT_DIR = "output/dreamdox_academy_test_v4_admin"
CREDENTIALS = {"email": "admin2@dreamdocs.ru", "password": "AdminPass123!"}
...
async def main():
    ...
    try:
        ...
        try:
            ...
            raw, _ = await ss(browser, "01_after_login", None)
        add_step("Логин", screenshot=raw)
        txt = await page.evaluate("() => document.body.innerText")
        if "невер" in txt.lower() or "invalid" in txt.lower() or "ошибка" in txt.lower():
            add_bug("Авторизация не удалась", "critical", "Админские креды не проходят авторизацию.", screenshot=raw, module="auth")
            return
        ...
        try:
            ...
            raw, _ = await ss(browser, "02_dashboard", None)
        add_step("Дашборд", screenshot=raw)
        txt = await page.evaluate("() => document.body.innerText")
        if "админ" in txt.lower() or "admin" in txt.lower():
            add_step("Роль админа подтверждена", details="На дашборде найдены признаки админ-роли")
        ...
        try:
            ...
            raw, _ = await ss(browser, "03_courses", None)
        ...
        try:
            ...
            raw, _ = await ss(browser, "04_dreamdocs_top", None)
        ...
        # 8. Admin panel / создание курса
        await browser.navigate(f"{BASE_URL}/admin/courses")
        await asyncio.sleep(2)
        raw, _ = await ss(browser, "13_admin_courses", None)
        add_step("Админка курсов", screenshot=raw)
        txt = await page.evaluate("() => document.body.innerText")
        if "404" in txt or "not found" in txt.lower() or "доступ запрещен" in txt.lower() or "forbidden" in txt.lower():
            add_bug("Админ-панель недоступна", "high", "Админ не может открыть /admin/courses", screenshot=raw, module="admin")
        elif "курс" in txt.lower() or "создать" in txt.lower():
            add_step("Админ-панель доступна")

        # 9. Car Repair через прямой URL
        await browser.navigate(f"{BASE_URL}/courses/72")
        await asyncio.sleep(2)
        raw, _ = await ss(browser, "14_car_repair", None)
        ...
        if "404" in txt or "не найден" in txt.lower() or "not found" in txt.lower():
            ...
            add_bug("Car Repair требует записи", "low", "Админ не заэнроллен в курс 72.", screenshot=raw, module="course")
        ...
        if forum_ok:
            await asyncio.sleep(2)
            raw, _ = await ss(browser, "15_forum_opened", None)
        ...
        try:
            els = await browser.get_interactive_elements()
            raw, _ = await ss(browser, "99_error", els)
        except Exception:
            raw, _ = await ss(browser, "99_error", None)
