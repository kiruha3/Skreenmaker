...
OUTPUT_DIR = "output/dreamdox_academy_test_v2"
...
async def main():
    ...
    try:
        ...
        # Кликаем по тексту содержащему "Тест" или "Самопроверка"
        test_clicked = False
        ...
        if test_clicked:
            ...
            start_ok = await safe_click(browser, "Начать тест", by="text")
            if start_ok:
                await asyncio.sleep(2)
                raw, _ = await ss(browser, "09_quiz_started", None)
                add_step("Тест начат", screenshot=raw)

                # Выбираем первый вариант ответа (radio)
                try:
